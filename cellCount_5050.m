...
for i = 1:2
    for f = 1:2
        cd Example_images/
        filename1 = ['220311_5050_1_cs' num2str(f) '_20X_' num2str(i) '_DAPI.tif'];
        ...
        for cell = 2: num
            ...
            if length(find(one == 1))>0.4*length(one)
                neuron = neuron+1;
        ...
        %save('counts_5050_crop_.mat', 'neuron', 'NeuronLoc', 'astro', 'AstroLoc', 'maskNeu', 'maskAs', 'map2GR', 'dapiBW', 'dapiNoise')
        %%
        %GFAP
        ...
        for cell2 = 2: astro
            ...
            if length(find(one_1 == 1))>0.03*length(one_1)
                astro2 = astro2+1;
